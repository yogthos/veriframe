...
/-!
# Hadwiger–Nelson: $\chi(\mathbb{R}^2) \geq 4$ via the Moser spindle
...
points at distance exactly $1$ share a colour. Currently $5 \leq \chi(\mathbb{R}^2) \leq 7$,
with both bounds open.

This file establishes the lower bound $\chi(\mathbb{R}^2) \geq 4$ via the **Moser spindle**
(Moser–Moser, 1961): an explicit $7$-vertex unit-distance graph that is not $3$-colourable.
