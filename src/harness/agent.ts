  ...
  switch (name) {
    ...
    case "audit": {
      ...
      const auditorPrompt = [
        ...
        "",
        "**D. Thesis-vs-artifact alignment (uses the registered thesis above):**",
        ...
        "",
        "**E. Novelty / re-derivation check (NEW — catches dressed-up restatements of known results):**",
        "  E1. Re-read the original problem statement carefully, including any sections labelled \"prior verified results\", \"already proved\", \"verified knowledge to start from\", \"DO NOT reproduce\", or similar. The problem prompt LISTS what's already known. Does the proposed answer or verified artifact substantively re-derive any of those listed results, even with a DIFFERENT parameterization, framing, or technique? Re-derivation of known results dressed in new clothes does not count as progress.",
        "  E2. **Specifically check residue-class coverage**: if the prior results say something like \"n ≡ k (mod m) is covered by construction X\" and the proposed answer claims a NEW universal construction for the SAME or a SUB-residue class (e.g., n ≡ 5 mod 12 is a sub-residue of n ≡ 1 mod 4 already-covered classes), flag E-fail and name the prior result being re-derived.",
        "  E3. **Specifically check construction equivalence**: if the proposed answer's construction (x, y, z) numerically matches the prior catalogued construction's (x, y, z) for the same input — possibly via a renaming of parameters — that is a re-derivation, not novelty. Compute one or two small instances and compare.",
        "  E4. If the result is mathematically novel — addressing territory NOT in the prior catalog, or producing a strictly stronger / more general statement, or formalizing a previously informal argument in a new logical system — confirm that explicitly and pass. Be as harsh on novelty claims as on encoding bugs.",
        "",
        "Respond in this exact format:",
        "  Verdict: AUDIT PASSED  — OR — Verdict: AUDIT FAILED",
        "  Reasoning: <5-8 sentences covering all five categories>",
        "",
        "If AUDIT FAILED, name the specific failure pattern (e.g., 'B2: refutes a guessed mapping, not the actual paper's method' or 'C2: answer addresses small primes only, problem asked for general proof' or 'D2: thesis claims universal proof but artifact verifies p ∈ {5,13,17} only' or 'E2: claimed novel construction for p ≡ 5 mod 12 but that residue class is already in the prior Lean catalog under construction (3t+2, (t+1)n, (3t+2)(t+1)n)') and recommend the concrete fix.",
      ].join("\n");
