    ...
    case "audit": {
      ...
      const auditorPrompt = [
        ...
        target.model
          ...
          : "",
        "",
        "Audit checklist — five categories. For each, decide pass/fail; one failure anywhere means AUDIT FAILED.",
        "",
        "**A. Encoding soundness.** Does the SMT/Lean code actually capture the claim? Watch for: vacuous SAT (formula constrains a strict subset of the property), missing distinctness on variables that should differ, mis-scoped quantifiers, polarity flips (asserting P vs. (not P)), and SAT witnesses that don't satisfy every claim-relevant constraint. Fail if the encoding is unsound.",
        "",
        "**B. Verdict-vs-answer alignment.** Does the proposed answer accurately describe what the engine verified? Catch the 'I read Z3 SAT as proving X but it actually shows Y' pattern. If the answer cites a counterexample or refutation of external work, confirm the right thing was actually refuted (not a guessed mapping). Confidence in the answer must match the artifact's strength — no over-claiming partial results as full theorems.",
        "",
        "**C. Thesis-vs-problem reflection.** Re-read the original problem. Does the proposed answer address what was asked, or a related-but-easier question? Are scope gaps acknowledged honestly? Is novelty / scope overclaimed in the framing?",
        "",
        "**D. Thesis-vs-artifact alignment.** Does the verified artifact advance one of the registered sub-claims (name it), and is the artifact's scope CONSISTENT with the thesis's scope? Critical finite-vs-universal check: if the thesis is universal (e.g., 'for all primes p ≡ 1 mod 4...') but the artifact verifies only instances (e.g., 'p=5 has solution (2,5,10)'), this is a D-fail UNLESS the proposed answer explicitly scopes itself to the verified instances. Also check that the registered non-finite justification actually holds for the artifact.",
        "",
        "**E. Novelty / re-derivation.** Re-read the problem's 'prior verified results / DO NOT reproduce' sections. Does the proposed answer substantively re-derive any catalogued result with a different parameterization, framing, or technique? **Compute one or two small instances** of the proposed (x, y, z) construction and compare against the prior catalog's construction for the same input — if they cover the same residue class (or a sub-residue of one), it's a re-derivation. Be as harsh on novelty claims as on encoding bugs.",
        "",
        "Respond in this exact format:",
        "  Verdict: AUDIT PASSED  — OR — Verdict: AUDIT FAILED",
        "  Reasoning: <one short paragraph, one sentence per category>",
        "",
        "If AUDIT FAILED, name the specific failure (letter + one-line description) and recommend the concrete fix. Examples: 'B2: refutes a guessed mapping, not the paper's method'; 'D2: universal thesis but artifact verifies finite primes'; 'E: claimed novel construction for p ≡ 5 mod 12 but already covered by the Lean catalog under (3t+2, (t+1)n, (3t+2)(t+1)n)'.",
      ].join("\n");
